...
namespace solution.Reserve
{
    public partial class Views : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (Session["USERTYPE"].ToString() == ((int)EnumOR.UserType.EnquireOnly).ToString())
            {
                Response.Redirect("/EnquirePrice/Default");
        ...
        private List<ORHEADERVO> loadORHeader(string CODE)
        {
            ...
            foreach (ORHEADERVO ORHEADERVO in templstORHEADERVO)
            {
                ...
                //    _lstAPPOINTMENTVO.Add(APPOINTMENTVO);
                //}
                if (_lstAPPOINTMENTVO.Count > 0 && _lstAPPOINTMENTVO[0].ConfirmStatusType != 6)
                {
                    //--------------------------//
        ...
        private void checkcolorrow(DateTime iORDate, DateTime iCreDate, GridViewRowEventArgs e)
        {
            ...
            if (iordate == 1)
            {
                int itime = 1530 - int.Parse(iCreDate.ToString("HHmm"));
                if (itime <= 0)
                {
        ...
        public void loadvalueDetail(GridViewRow row)
        {
            try
            {
                ...
                if (_lstORHEADERVO.Count > 0)
                {
                    ...
                    SETUPORROOMVO eSTUPORROOMVO = new SETUPORROOMVO();
                    eSTUPORROOMVO.CODE = _lstORHEADERVO[0].ORRoom;
                    try { lblORRoom.Text = new BLSETUPORROOM(dbInfo).SearchByKey(eSTUPORROOMVO)[0].Name; } catch { }
                    ...
                    lblPrediag.Text = _lstORHEADERVO[0].Prediag;
        ...
        {
            DateTime now = DateTime.Now;
            hdDate.Value = DateTime.Now.AddDays(1).ToString("dd/MM/yyyy");
            loadvalue();
        }
