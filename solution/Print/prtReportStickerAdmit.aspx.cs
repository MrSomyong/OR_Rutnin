...
{
    public partial class prtReportStickerAdmit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request.QueryString["o"] != null)
                {
                    ...
                    string orid = Request.QueryString["o"];
                    string _date = Request.QueryString["d"];
                    DateTime Ordate = DateTime.Parse(_date);
                    //
                    ReportDocument crystalReport = new ReportDocument();
                    string pathorrom = Server.MapPath("/") + "/Report/Report/ORStickerAdmit.rpt";
                    crystalReport.Load(pathorrom);

                    DataSet dsCustomers = GetData(orid, Ordate);
        ...
        private DataSet GetData(string orid, DateTime Ordate)
        {
            DataSet ds = new DataSet();
            try
            {
                DataTable dtheader = new BLORHEADER(dbInfo).SearchByORID_DS(orid);
